...
When a REGISTERED VISUAL ASSETS block is present, use it as incremental visual state. Choose a background at scene start or when the background should change, and choose SCGs when the visible character set or a visible character's registered pose should change. Do not repeat unchanged background/SCG IDs on every segment; omitted visuals stay on screen. If every visible character leaves, set clear_characters=true. For background choice, prefer an exact location/variant match; if none exists, choose the registered background that is closest in place type, indoor/outdoor feel, time, season, or mood rather than leaving the scene without a background. If the current background is still reasonably usable, it may remain unchanged. If multiple authored staging choices are equally valid, prefer one that has an exact registered visual match instead of an equivalent unrepresented staging. Respect all current runtime controls in narration, dialogue, and character behavior when they are semantically relevant. For character SCGs, choose an asset that matches the intended semantic state. Scenario controls with predeclared finite options may also correspond to asset metadata fields; the engine can remap the chosen SCG to the current option values without another LLM call. Such visual remapping does not itself imply a change to identity, memory, relationships, or story state. Put only registered asset IDs in structured response assets; never put file paths there.
"""
